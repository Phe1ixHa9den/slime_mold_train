...
def main():
    ...
    parser.add_argument('--cond_dir', type=str, default="selected2", help='条件帧所在文件夹（按文件名排序）')
    ...
    parser.add_argument('--img_size', type=int, default=256, help='图像尺寸 (正方形)')
    parser.add_argument('--num_frames', type=int, default=10, help='要生成的帧数')
    parser.add_argument('--output_dir', type=str, default='./test_generated_selected_200_epoch', help='输出目录')
    parser.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    parser.add_argument('--num_timesteps', type=int, default=2000, help='扩散步数（需与训练一致）')
